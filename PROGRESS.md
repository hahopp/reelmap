...
> 최종 업데이트: 2026-06-18 · 기준 문서: `PRD.md`(v0.6), `DEVELOPMENT_PLAN.md`, `CLAUDE.md`
...
## ▶ 내일 이어서 (추천 시작점)
- **Vercel 배포** — 배포하다 기능 작업으로 전환했음. 가이드: ① `npx vercel` → ② 환경변수 6개(.env.local) Vercel에 등록 → ③ `vercel --prod` → ④ **카카오 플랫폼에 vercel 도메인 등록**(지도 뜨려면 필수).
- 그 외 백로그는 맨 아래 참고.
...
- 폰트 **Pretendard** · PostHog(미연동) · Vercel(미배포)
...
### 공개(비로그인)
- **`/` 홈** — 큐레이션 시드맵 카드 그리드(반응형 1→2→3열), 커버(이미지/그라데이션+이모지) + **hover 확대**.
...
### 코어 모듈
- `lib/instagram.ts`(URL→post_id, vitest 16) · `lib/places/`(searchPlaces·transcoord·coord2address) · `lib/kakao-url.ts`(map.kakao URL 파싱) · `lib/tags.ts`(태그 파싱)
- `lib/maps.ts` · `lib/pins.ts` · `lib/public-maps.ts` · `lib/supabase/server.ts`
- `components/map/MapView.tsx`(카카오 SDK·번호마커·focus 이동·마커 클릭) · `app/m/[share_token]/PublicMapView.tsx`(인터랙티브 본문) · `app/explore/TagFilter.tsx`(basePath 공용) · `components/icons/instagram.tsx`
...
- 스모크(`scripts/`): check-db · smoke-map/kakao/register/public/cover · list-maps · seed-demo · demo-tag · backfill-address
- `npm run dev` · 마이그레이션은 Supabase SQL Editor · 어드민 비번 = `.env.local` ADMIN_PASSWORD · 공개 URL: `node scripts/list-maps.mjs`

---

## 다음 후보 (백로그)
- ⭐ **Vercel 배포**(위 "내일 이어서") · **소비자 핵심 루프**(방문자가 링크 붙여넣기→위치, 가설 B) · **소셜 로그인**(Phase 3)
