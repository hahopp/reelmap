...
5. **내 지도 관리(증분 4 · 전체 보기 + 지도 필터)**: `app/my`(`MyMapClient`, 클라) — `getMyMapsAction`(토큰 검증)으로 **모든 지도+핀+설명을 1회 로드**(`getMyMapsWithPins`, 왕복 상수 `.in()`) → **전체 보기 기본**(전 지도 장소 placeId 합집합 + 🗺 소속 지도 배지) + **지도 칩=필터**(클라 즉시·네트워크 0). **`MapView`는 `markers` prop 변경에 반응**(필터 전환 시 지도 마커 갱신). 지도 선택 시 관리 노출: 설명 표시·설정(`MapNameDialog` 이름+설명)·`AddPlaceDialog`·공유·삭제. 핀 제거 = `removePinFromMyMap`(소유권 검증 → `map_pin` 삭제 + 해당 `submission`의 내 `selection`도 삭제 = 투표 회수). 공유 = `/m/{share_token}` 링크 복사. (전체 보기는 둘러보기 전용)
