...
lib/                          # 도메인 로직 (대부분 'server-only')
  ...
  consumer.ts                 # 소비자 담기 쓰기 + 내 지도 조회/핀 제거(토큰 검증 + service_role). saveCandidate/savePlace·addPlaceToMyMap(개인핀)·addPlaceFromReel(릴 연결 후보 source='user')·upsertPlaceDedup(공용)·listMyMaps(목록·카운트)·getMyMapPins(지도별 핀, 지연 로드)
...
5. **내 지도 관리(증분 4 · 하이브리드 지연 로드)**: `app/my`(`MyMapClient`, 클라) — `getBrowserSupabase` 세션 → `listMyMapsAction`으로 **칩 목록(제목·카운트)만 즉시** → 지도를 **처음 열 때만** `getMyMapPinsAction`(토큰 검증)으로 그 지도 핀 지연 로드(로딩 표시), **재방문은 `pinsByMap` 캐시로 즉시(네트워크 0)**. 초기 가볍고 확장에 강함. `MapExplorer`(우상단 `RemovePinButton`, `loading` 슬롯) 재사용 — **`MapView`는 `markers` prop 변경에 반응**(전환 시 지도 마커 갱신). 핀 제거 = `removePinFromMyMap`(소유권 검증 → `map_pin` 삭제 + 해당 `submission`의 내 `selection`도 삭제 = 투표 회수). 공유 = `/m/{share_token}` 링크 복사.
