...
> **안정성**: 🟡 반-안정 — 구조가 바뀌면 갱신.
> **최종수정**: 2026-06-21
> **연관**: [STATUS.md](STATUS.md) 진행 상태 · [PRD.md](PRD.md) 기획(데이터모델 7장) · [decisions/0002-instagram-ingest-pipeline.md](decisions/0002-instagram-ingest-pipeline.md) 인제스트 · [../CLAUDE.md](../CLAUDE.md) 규칙 · [../AGENTS.md](../AGENTS.md) Next 16 주의
...
app/                          # 라우트 (App Router)
  ...
  admin/                      # 운영자 시드 도구
    ...
    (dashboard)/              #   세션 보호 그룹
      layout.tsx · page.tsx · actions.ts          # 지도 CRUD + 인제스트 네비
      maps/[id]/…             #   핀/장소 등록(PlaceRegister 3탭)
      capture/               #   ① 포착 전용(raw 입력) — page·CaptureForm·actions
      review/                #   ③ 검토·확정 — page·ReviewCard·RefineButton·actions
      places/                #   ④ 장소·편성(태그검색·일괄 담기) — page·PlacesManager·actions
  api/captures/              # 인제스트 API(Bearer): route(GET목록) · [id]/route(PATCH) · [id]/refine(POST)
...
lib/                          # 도메인 로직 (대부분 'server-only')
  ...
  refine.ts                   # Claude 추출(Sonnet 4.6) + 카카오 후보 자동매칭
...
사람이 인스타 액션(팔로우/댓글/확인)은 수동, 저장·정제만 자동. ToS상 자동 인스타 호출 없음.
1. **① 포착**: `capture/`(raw 입력 전용, 모바일) → `lib/captures.ts` → `instagram_capture(status='raw')`.
2. **② 추출**: `/api/captures`(Bearer, 외부 에이전트 루프) 또는 `review`의 'AI 정제' 버튼 → `lib/refine.ts`(Claude Sonnet 4.6 구조화 + 카카오 후보 자동매칭) → `extracted jsonb`, status=`refined`.
3. **③ 확정**: `review/ReviewCard`(좌표·카테고리·태그 확정, 카카오 재검색) → `lib/pins.ts registerSeedPlace`(**지도 없이** content+place+submission) → capture=`confirmed`+`place_ids`.
4. **④ 편성**: `places/PlacesManager`(태그 AND 검색·체크박스) → `addPlacesToMap`(map_pin 벌크 upsert).
