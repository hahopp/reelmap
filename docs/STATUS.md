...
- **`/my` 지도 전환 구조 개선(성능·UX, 코드·로컬 green · 런타임 스모크 OK)**: 전환마다 서버액션 1회(~6왕복 직렬)로 ≥1초 걸리던 것 → **하이브리드 지연 로드**(2026-06-29 사용자 결정). 진입 시 **칩 목록(제목·카운트)만 즉시**(`listMyMaps`), 지도는 **처음 열 때만 그 지도 핀을 지연 로드→로딩표시**(`getMyMapPins`→`listMapPins`), **재방문은 클라 캐시로 즉시**(네트워크 0). 초기 로드 가볍고 지도·핀 많아져도 확장에 강함(연 지도만 로드). `MyMapClient` = `maps[]`(칩) + `pinsByMap`(지연 캐시) + 지도별 로딩/에러. 장소 추가/제거는 해당 지도만 로컬 갱신. + **MapView 마커 리렌더 버그 수정**(아래 §5) — `markers` prop 변경에 반응(인앱 지도 전환 시 지도 마커가 실제로 갱신). 신규 `getMyMapPins`/`getMyMapPinsAction`, `MapExplorer loading` 슬롯. 스모크 `scripts/smoke-mymaps.mjs`. (남은 레버: 각 지도 조회 자체를 더 빠르게 — `getUser()` 원격검증 → 로컬 JWT / Vercel↔Supabase 리전 일치 — 후속)
