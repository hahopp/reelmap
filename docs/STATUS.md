...
- **장소 카드 인스타 링크 = 라벨 버튼 + 출처 릴 병합(UX, 코드·로컬 green)**: 사용자 요청(2026-07-01). 공용 `MapExplorer` 카드의 인스타 링크를 **맨 아이콘 원 → 라벨 버튼("인스타에서 보기", 복수면 "인스타 N")** 으로. 표시 코드 = 장소 후보(`instaCodes`) **∪ 이 핀의 출처 릴(`contentId`)** (중복 제거) — 후보로 안 올라간 개인 핀의 출처 릴도 버튼으로 노출. `/my`·`/m`·`/explore` 공통 적용(`MapExplorer` 변경 + `MyMapClient toItem`에 `contentId` 전달). 태그는 기존대로 장소에 태그가 있을 때 `#태그` 칩으로 노출(편집 = 카드 ✎).
