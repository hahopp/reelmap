...
- **`/my` 전체 보기 + 지도 필터 + 지도 설명(UX, 코드·로컬 green · 스모크 OK)**: 사용자 요청 #1·2·3·4 반영(2026-06-29). ① **전체 보기 기본** — 내가 담은 모든 장소를 placeId로 합쳐 한 번씩(어느 지도 소속인지 🗺 배지). ② **지도 칩 = 필터**(클라 즉시·네트워크 0) — 모든 지도+핀을 1회 로드(`getMyMapsWithPins`, 왕복 상수 `.in()` 배치)하고 전환/필터·전환은 클라에서. ③ **지도 설명**(`map.description` 기존 컬럼) — 생성/설정 다이얼로그에 이름+설명, 단일 지도 뷰에 표시. ④ **MapView 마커 리렌더 버그 수정**(아래 §5) — `markers` prop 변경 반영(필터 전환 시 지도 마커 갱신). 카드의 인스타 링크 클릭→이동은 기존(=#3 충족). 전체 보기는 둘러보기 전용(관리=지도 선택 시). 신규/변경: `getMyMapsWithPins`/`getMyMapsAction` · `MapExplorer mapNames`(소속 지도 배지) · `MapNameDialog`(이름+설명) · `create/renameMyMap`(설명) · 스모크 `scripts/smoke-mymaps.mjs`. **다음**: #5 커버이미지(Storage 버킷)·#6 공유범위 3단계(visibility 마이그레이션) — 운영자 작업 필요. (perf 남은 레버: `getUser()` 원격검증 → 로컬 JWT / Vercel↔Supabase 리전 일치 — 후속)
- **장소 추가 + 위치 선택 UX 개선(코드·로컬 green)**: 사용자 요청 반영. ① **컨텐츠 링크 우선**(`AddPlaceDialog` 맨 위에 "릴/게시물 링크(선택)" → 위치 선택 → 이름) — **링크 있으면 릴 연결 후보**(`addPlaceFromReel`, 남들도 발견·확인 단계에 `✓ 릴 연결됨`) / **없으면 개인 핀**(분기, 모드 토글 없이 옵셔널 입력+헬퍼). ② **검색 무결과 → "지도에서 직접 선택" CTA**(map 탭 전환 유도). ③ **지도 클릭 시 핀 표시**(MapView markers prop, 저번 마커 수정 활용) + ④ **클릭 위치 주소 즉시 역지오코딩 표시**. `LocationPicker` 개선이라 `/find` 무후보 추가(`AddPlaceFromReel`)도 동일 적용. (다음: 편집을 팝업→**카드 인라인 확장**으로 전환)
- **장소 카드 인스타 링크 = 라벨 버튼 + 범위(내 지도=내 것만)(UX, 코드·로컬 green)**: 사용자 요청(2026-07-01). 공용 `MapExplorer` 카드의 인스타 링크를 **맨 아이콘 원 → 라벨 버튼("인스타에서 보기", 복수면 "인스타 N")** 으로. **범위(`instaScope`) 맥락별**: `/my`=**`own`**(내가 등록한 릴만) · `/m`·`/explore`=**`all`**(전부, 발견 맥락). 두 경우 모두 **이 핀의 출처 릴(`contentId`)은 항상 포함**. '내 것만' = 내가 제출한 submission(`submitted_by=나`) ∪ 출처 릴 — 데이터층에서 `ownInstaCodes` 별도 산출(카드 표시용), `instaCodes`(전체)는 ✎ 편집/관리용으로 유지. 변경: `MapExplorer instaScope`/`ExplorerItem.ownInstaCodes` · `PinRow.ownInstaCodes` · `getMyMapsWithPins`(내 submission 배치 조회) · `MyMapClient`. 태그는 기존대로 장소에 태그 있을 때 `#태그` 칩(편집 = 카드 ✎).
- **내 장소 편집 + 칩 줄바꿈(UX, 코드·로컬 green)**: `/my` 단일 지도 뷰의 각 장소 카드에 ✎ 편집(`EditPlaceDialog`). **메모(핀 단위, 항상)** + **내가 만든 장소(`place.created_by`=나)면 태그·릴 링크** 편집(공유 카탈로그라 남이 만든/시드 장소는 메모만 — `editable` 플래그 = PinRow에 created_by 비교 추가). 릴은 add/remove(=submission `source='user'` upsert/delete, selection FK cascade). 소유권 게이트 = `assertPinOwner`(지도 소유)·`assertPlaceCreator`(장소 생성자). 신규 데이터층 `updatePinNote`/`updateMyPlaceTags`/`addMyPlaceReel`/`removeMyPlaceReel` + 액션. 지도 칩 줄은 가로 스크롤 → **줄바꿈(flex-wrap)**.
...
- [x] **MapView 마커 리렌더** ✅(2026-06-29): markers 구성을 `initMap()` 1회 → `markerKey` 의존 `useEffect`로 분리(이전 마커 정리 후 재구성). 인앱 지도 전환(`/my` 칩) 시 지도 마커가 실제 갱신. `/find`·`/m`·`/explore`·LocationPicker 무회귀(단일 셋은 1회 렌더)
